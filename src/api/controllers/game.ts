...
const save = async (req: any, res: Response) => {
  ...
  try {
    ...
    if (game) {
      return res
        .status(200)
        .send({ message: "Game successfully created", match: game });
    } else
      return res
        .status(500)
  ...
  try {
    ...
    gameT
      .then((succ: any) => {
        res
          .status(200)
          .send({ message: "Game Transactions successfully created" });
      })
      .catch((err: any) => {
        console.log(err);
...
const join = async (req: any, res: Response) => {
  ...
  try {
    ...
    if (game) {
      ...
      socket.emit(`${game.players[0]._id}`, {
        type: JOIN_SOLITAIRE_GAME,
        data: game,
      });
...
const unJoin = async (req: any, res: Response) => {
  try {
    ...
    if (game) res.status(200).send({ message: "You Are successfully joined" });
    else res.status(400).send({ message: "Please Verify your information!" });
  } catch (err: any) {
    console.log(err);
    res.status(500).send({ message: "An error has occurred!" });
...
const getProcess = async (req: any, res: Response) => {
  try {
    res.status(200).send({ data: await matchService.getProcess() });
  } catch (err: any) {
    console.log(err);
    res.status(500).send({ message: "An error has occurred!" });
  }
...
const addUpdateScore = async (req: Request, res: Response) => {
  try {
    ...
    if (score) res.status(200).send({ message: "Updated successfully" });
    else res.status(400).send({ message: "Please Verify your information!" });
  } catch (err: any) {
    console.log(err);
    res.status(500).send({ message: "An error has occurred!" });
...
const getScoreByGameId = async (req: Request, res: Response) => {
  try {
    ...
    if (score) return res.status(200).send({ data: score });
    else
      return res
        .status(400)
        .send({ message: "Please Verify your information!" });
